      ...
      <div class="row">
        <form method="get" action='' class="col-sm-12 col-md-12">
            <div id="column-filter_filter" class="dataTables_filter">
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class=" mt-3">
                            ...
                            <a href="{{url('portal/report/service')}}"  class="btn btn-primary">Clear</a>
                ...
                <a href="{{url('portal/report/service_request')}}"  class="btn btn-primary">Clear</a> --}}
            </div>
        </form>
      </div>
               ...
               <?php $i = $list->perPage() * ($list->currentPage() - 1); ?>
                @foreach($list as $datarow)
                    ...
                    <tr>
                        ...
                        <td>{{number_format($datarow->admin_commission, 2, '.', '')}}</td>
                        <td>{{number_format($datarow->vendor_commission, 2, '.', '')}}</td>
                        <td>{{ number_format($datarow->grand_total - $datarow->vat + $datarow->discount - $datarow->service_charge, 2, '.', '') }}</td>
                        <td>{{number_format($datarow->discount, 2, '.', '')}}</td>
                        <td>{{number_format($datarow->vat, 2, '.', '')}}</td>

                        <td>{{number_format($datarow->service_charge, 2, '.', '')}}</td>
                        <td>{{number_format($datarow->grand_total, 2, '.', '')}}</td>
                        <!-- <td>{{ number_format(($datarow->hourly_rate * $datarow->qty) + $datarow->vat + $datarow->service_charge - $datarow->discount, 2, '.', '') }}</td> -->
                        <td>
                          {{payment_mode($datarow->payment_mode)}}
                        </td>
                        <td>{{ service_order_status($datarow->order_status) }}</td> 
