    ...
    <div class="card-body">
      <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class=" mt-3">
                            ...
                            <a href="{{url('admin/report/activities')}}"  class="btn btn-primary">Clear</a>
        ...
        <table class="table table-condensed table-striped" id="customers_listing">
            ...
            <tbody>
                <?php $i = $logs->perPage() * ($logs->currentPage() - 1); ?>
                @foreach($logs as $datarow)
                    <?php $i++ ?>
                    <tr>
                        <td>{{$i}}</td>
                        <td>{{ $datarow->user->name?? "Guest" }}</td>
                            <td>{{ $datarow->activity }}</td>
                            <td>{{ $datarow->ip_address }}</td>
        ...
        <span>Total {{ $logs->total() }} entries</span>
            ...
            <div class="col-sm-12 col-md-12 pull-right">
                ...
                {{-- {!! $logs->links('admin.template.pagination') !!} --}}
                {!! $logs->appends(request()->input())->links('admin.template.pagination') !!}
                </div>
            </div>
        </div>
    </div>
