...
@for ($i = 0; $i < count($combinations); $i++)
    ...
    @if ( array_key_exists($i, $product_variations) !== FALSE )
     <?php    $t_variant_id = $product_variations[$i]->product_attribute_id;
        ...
        $short_desc_ar = $product_variations[$i]->product_desc_short_arabic;
        $full_desc = $product_variations[$i]->product_full_descr;
        $full_desc_ar = $product_variations[$i]->product_desc_full_arabic;
                   ...
                   <div class="col-lg-3 d-none">
                    ...
                    <div class="col-lg-12">
                        <div class="form-group profile-form">
                           ...
                           <textarea name="product_variant_short_desc[<?=$input_index?>]" data-editor="ck" id="des{{$start_index+$i}}" class=" ckeditor form-control description_fld">{{$short_desc}}</textarea>
                        </div>
                    </div>
                     <div class="col-lg-12">
                        <div class="form-group profile-form">
                            <label>Short Description (Arabic)</label>
                           <textarea name="product_variant_short_desc_ar[<?=$input_index?>]" data-editor="ck" id="des{{$start_index+$i}}" class="rtl-text ckeditor form-control description_fld text-right">{{$short_desc_ar}}</textarea>
                        </div>
                    </div>
                    <div class="col-lg-12">
                        <div class="form-group profile-form">
                            <label>Description </label>
                           <textarea name="product_variant_full_desc[{{$input_index}}]" data-editor="ck" id="desfull{{$start_index+$i}}" class="ckeditor form-control description_fld">{{$full_desc}}</textarea>
                        </div>
                    </div>
                     <div class="col-lg-12">
                        <div class="form-group profile-form">
                            <label>Description (Arabic)</label>
                           <textarea name="product_variant_full_desc_ar[{{$input_index}}]" data-editor="ck" id="desfull{{$start_index+$i}}" class="rtl-text ckeditor form-control description_fld text-right">{{$full_desc_ar}}</textarea>
                        </div>
                    </div>
                            ...
                            <div class="upload-img-product-items" data-variant-id="<?php echo $t_variant_id ?>">
                                <?php if (! empty($t_variant_images) ): ?>
                                    <?php foreach ($t_variant_images as $t_name): ?>
                                        <?php
                                        $exist = file_exists(config('global.product_image_upload_dir'). "/{$t_name}");
                                        if ( !empty($t_name) ) {
                                            $t_img = get_uploaded_image_url( config('global.product_image_upload_dir').$t_name);//url(config('global.upload_path') . "/" . config('global.product_image_upload_dir')."/".$t_name) ;
                                        } else {
                                ...
                                <div class="uploaded-prev-imd input-upload-file-column" <?php echo ($readonly ? 'style="display:none;"' : '') ?>>
                                    <div class="image_wrap">
                                        <label class="Pic_upload">
                                            <input type="file" counter="{{$input_index}}" name="product_var_temp_image[<?=$input_index?>][]" class="upload_pro form-control" data-role="product-img-upload" multiple data-parsley-fileextension-message="Only files with type jpg,png,gif,jpeg are supported"  />
                                            <i class="ti-plus"></i>
                                        </label>
                                    </div>
                                </div>
...
<div class="upload-product-video">
                    ...
                    @php
                        ...
                        //$url = print_r($product->video);die("here");
                    @endphp

                    <div id="video-preview-container" class="uploaded-prev-video" style="{{ empty($videoURL) ? 'display:none;' : '' }}">
                        <video data-fancybox href="{{ $videoURL }}"  id="video-preview" width="300" height="200" controls>
                            @if(!empty($videoURL))
                                <source src="{{ $videoURL }}" type="video/mp4">
                            @endif
                            Your browser does not support the video tag.
                        </video>
                        <div class="del-product-video" data-role="product-video-trash" role="button">
                            <i class="far fa-trash-alt"></i> Remove
                        </div>
                    </div>

                    {{-- Upload input --}}
                    <div class="input-upload-file-column" {{ $readonly ? 'style=display:none;' : '' }}>
                        <div class="image_wrap">
                            <label class="Pic_upload">
                                <input
                                    type="file"
