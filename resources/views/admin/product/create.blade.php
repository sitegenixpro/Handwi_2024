...
@section('content')
    ...
    <style>
        ...
        .image-preview {
            width: 100%;
            height: 100%;
            object-fit: cover; /* Ensures the image is cropped correctly without distortion */
        }
...
.ck-editor__editable {
    color: #000 !important; /* or any readable color */
}
.rtl-text {
    direction: rtl;
    text-align: right;
                ...
                <div class="card mb-2">
                    <div class="card-body">
                        ...
                        <div class="col-md-6 form-group ">
                            <label>Tags<b class="text-danger">*</b></label>
                            <select class="form-control jqv-input select2" data-role="select2" name="tags[]" multiple="multiple"  required
                                data-parsley-required-message="Select Tags"  >
                                <option value="">Select Tags</option>
                                @foreach ($tags as $tag)
                                    <option value="{{$tag->id }}"  {{in_array($tag->id,$tag_ids)?'selected':''}} >{{ $tag->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-12 form-group">
                         ...
                         <div class="col-md-12 form-group">
                            <label>{{$p_name}} Name (Arabic)<b class="text-danger">*</b></label>
                            <input type="text" name="product_name_arabic" required placeholder="Enter {{$p_name}} Name" class="form-control jqv-input text-right" data-jqv-required="true" value="{{ $name_ar }}">
                        </div>
                        ...
                        <div class="col-lg-3">
                        <label class="container ml-5">New Arrival
                                <input hidden="" name="new_arrival" type="checkbox" value="1" @if(!empty($product->new_arrival)) @if($product->new_arrival == 1) checked @endif @endif data-parsley-multiple="featured">
                                <span class="checkmark"></span>
                            </label>
                        </div>

                        <div class="col-lg-3">
                            <label class="container ml-5">Emirati Heritage
                                    <input hidden="" name="for_you" type="checkbox" value="1" @if(!empty($product->for_you)) @if($product->for_you == 1) checked @endif @endif data-parsley-multiple="featured">
                                    <span class="checkmark"></span>
                                </label>
                            </div>
                            <div class="col-lg-3">
                                <label class="container ml-5">Trending
                                        <input hidden="" name="trending" type="checkbox" value="1" @if(!empty($product->trending)) @if($product->trending == 1) checked @endif @endif data-parsley-multiple="featured">
                                        <span class="checkmark"></span>
                                    </label>
                                </div>

                                <div class="col-lg-3">
                                <label class="container ml-5">Personalization Option
                                    <input hidden="" name="personalisation" type="checkbox" value="1" @if(!empty($product->personalisation)) @if($product->personalisation == 1) checked @endif @endif data-parsley-multiple="featured">
                                    <span class="checkmark"></span>
                                </label>
                                </div>
                                
                                <div class="col-lg-3">
                                <label class="container ml-5">Explore
                                    <input hidden="" name="explore" type="checkbox" value="1" @if(!empty($product->explore)) @if($product->explore == 1) checked @endif @endif data-parsley-multiple="featured">
                                    <span class="checkmark"></span>
                                </label>
                                </div>
                        ...
                        <div class="col-md-12 form-group">
                            <label>Shipment And Policies<b class="text-danger"></b></label>
                            <textarea type="text" name="shipment_and_policies" class="form-control jqv-input ckeditor" data-jqv-required="true"
                                >{{ $shipment_and_policies }}</textarea>
                        </div>
                        <div class="col-md-12 form-group">
                            <label>Shipment And Policies(Arabic)<b class="text-danger"></b></label>
                            <textarea type="text" name="shipment_and_policies_ar" class="form-control jqv-input text-right ckeditor rtl-text" data-jqv-required="true"
                                >{{ $shipment_and_policies_ar }}</textarea>
                        </div>
                                        ...
                                        @foreach ($specs as $spec)
                                            <div class="row">
                                                <div class="col-md-3 form-group">
                                                 ...
                                                 <div class="col-md-3 form-group">
                                                    <input type="text" name="spec[{{ $i }}][title_ar]" placeholder="Title (AR)"
                                                        value="{{ $spec->title_ar ?? '' }}" class="form-control jqv-input text-right"
                                                        data-jqv-required="true">
                                                </div>
                                                <div class="col-md-3 form-group">
                                                ...
                                                <div class="col-md-2 form-group">
                                                    <input type="text" name="spec[{{ $i }}][description_ar]" placeholder="Unit (AR)"
                                                        class="form-control jqv-input text-right" value="{{ $spec->description_ar ?? '' }}">
                                                </div>
                                                <div class="col-md-1">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="top-bar d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <button class="btn btn-primary d-inline-flex align-items-center" type="button" data-role="add-item">
                                        <i class="flaticon-plus-1 mr-2"></i>Add
                                    </button>
                                </div>
                                ...
                                <!-- Item Holder -->
                                <div id="item-holder">
                                    @if (!empty($prod_features))
                                        <?php $j = 0; ?>
                                        @foreach ($prod_features as $item)
                                            <div class="row align-items-center mb-3">
                                                ...
                                                <div class="col-md-3 form-group">
                                                    <input type="text" name="item[{{ $j }}][name_ar]" placeholder="Name (AR)"
                                                        value="{{ $item->feature_title_ar ?? '' }}" class="form-control jqv-input text-right">
                                                </div>
                
                                                <!-- Feature Dropdown -->
                                                <div class="col-md-4 form-group">
                                                    <select data-url="{{ url('admin/sellers_by_categories') }}" 
                                                            class="form-control jqv-input product_catd select2"
                                                            name="item[{{ $j }}][feature_ids]" 
                                                            data-role="select2" 
                                                        ...
                                                        @foreach ($all_features as $feature)
                                                            <option value="{{ $feature->id }}" 
                                                                {{ $item->product_feature_id == $feature->id ? 'selected' : '' }}>
                                                                {{ $feature->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col-md-2 text-center">
                                                    <button type="button" class="btn btn-warning" data-role="remove-item">
    ...
    const html = `
        <div class="row">
            <div class="col-md-3 form-group">
             ...
             <div class="col-md-3 form-group">
                <input type="text" name="item[${itemCounter}][name_ar]" placeholder="Name (AR)"
                    class="form-control jqv-input text-right">
            </div>
            <div class="col-md-4 form-group">
                ${features_html}
            </div>
    ...
    $('.select2').select2();
        ...
        $('[data-role="add-spec"]').click(function() {
            ...
            var html = '<div class="row">' +
                ...
                '<div class="col-md-3 form-group">' +
                '<input type="text" name="spec[' + counter +
                '][title_ar]" placeholder="Title (AR)" class="form-control jqv-input text-right" data-jqv-required="true">' +
                '</div>' +
                '<div class="col-md-3 form-group">' +
                '<input type="text" name="spec[' + counter +
                ...
                '</div>' +
                 '<div class="col-md-2 form-group">' +
                '<input type="text" name="spec[' + counter +
                '][description_ar]" placeholder="Unit (AR)" class="form-control jqv-input text-right" data-jqv-required="true">' +
                '</div>' +
                '<div class="col-md-1">' +
        ...
        $('body').on('submit', '#admin-form', function(e) {
            ...
            $.each(form_uploaded_images, function (k, v) { 
                ...
                var videoInput = document.getElementById('product-video-input');
                if (videoInput && videoInput.files.length > 0) {
                    formData.append('product_video', videoInput.files[0]);
                }
    ...
    if (action == 'Create') {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // ✅ Reinitialize CKEditor on new elements
                initializeCKEditors();
            }
        });
    }
...
function initializeCKEditors() {
  document.querySelectorAll('.ckeditor').forEach((element) => {
    ...
    .then((editor) => {
      // Apply RTL + Right-align if it has the rtl-text class
      if (element.classList.contains('rtl-text')) {
...
$(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if ( data['status'] == 0 ) {
                            ...
                            $target.remove();
                            refreshUploadInputVisibility($imgList);
    ...
    function refreshUploadInputVisibility($container) {
        var currentCount = $container.find('.uploaded-prev-imd').not('.input-upload-file-column').length;

        if (currentCount < 5) {
            $container.find('.input-upload-file-column').show();
        } else {
            $container.find('.input-upload-file-column').hide();
        }
    }
...
$(document).ready(function () {
    ...
    const maxVideoSizeMB = 50; // Maximum allowed video size (50MB)

    // Using event delegation to make sure we target the element correctly
    $(document).on('change', '#product-video-input', function (e) {
        
        const file = e.target.files[0];
        const previewContainer = $('#video-preview-container');
        const previewVideo = $('#video-preview');
        ...
        // Size check: If video exceeds 50MB
    ...
    // Remove video preview when clicking the "Remove" button
    $(document).on('click', '.del-product-video', function () {
        $('#video-preview').attr('src', ''); // Clear the preview video
        $('#product-video-input').val(''); // Reset the file input
        $('#video-preview-container').hide(); // Hide the preview container
        $('#delete-product-video-flag').val('1');
    });
});
</script>
