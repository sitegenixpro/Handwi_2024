...
class LikeController extends Controller
{
    ...
    {
        $likes=$_GET['likes'] ?? '';
        $page_heading =( $likes)?"Likes": "Followers";
        $filter = [];
        $params = [];
        $params['search_key'] = $_GET['search_key'] ?? '';
        $search_key = $params['search_key'];
        $vendor=$_GET['vendor_id'] ?? '';
        $list = Likes::where('vendor_id',$vendor);
        if($vendor){
            $list=$list->where('vendor_id',$vendor); 
         }
        $list=$list->with('user')->orderBy('likes.created_at','desc')->paginate(10);
        
       
        return view("portal.likes.list", compact("page_heading", "list", "search_key"));
    }
