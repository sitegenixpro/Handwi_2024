...
class AuthController extends Controller
{
    ...
    public function email_login(Request $request)
    {
        ...
        $user = User::whereRaw("LOWER(email) = '$lemail'")->where('deleted', 0)->first();
        if ($user != null) {
            if (Hash::check($request->password, $user->password)) {
                $msg = '';
                if (!$user->active && $user->role==2) {
    ...
    public function social_login(Request $request)
    {
        ...
        if ($user = User::where('email', $request->email)->where("deleted", 0)->where(function ($query) {
            ...
            return response()->json([
                ...
                'user' => is_array($TempUser)?convert_all_elements_to_string($TempUser):$TempUser,
                // 'access_token' => $token,
            ], 201);
    ...
    public function signup(Request $request)
    {
        ...
        if ($request->password != $request->conf_password) {
            return response()->json([
                'status' => "0",
                'error' => (object) array(),
                'message' => trans('validation.password_mismatch'),
            ], 201);
        }
        ...
        if ($validator->fails()) {
            ...
            if ($user) {
                if ($request->otp == $user->password_reset_otp) {
                    ...
                    // $user->save();
                    $status = "1";
                    $message = trans('validation.otp_verified_success');
                } else {
