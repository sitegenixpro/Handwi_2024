...
class UsersController extends Controller
{
    ...
    {
        App::setLocale($request->header('language') ?? $request->language ?? 'en');
        $status = (string) 0;
        $message = "";
        $o_data = [];
        $errors = [];
        ...
        if ($validator->fails()) {
            ...
            if ($data->count() > 0) {
                ...
                if ($vendordata && $vendordata->logo) {
                    ...
                    $img = !$o_data->user_image ? "" : get_uploaded_image_url($o_data->user_image,'user_img_upload_dir');
                }
                
                $img = !$o_data->user_image ? "" : get_uploaded_image_url($o_data->user_image,'user_image_upload_dir');
                $o_data->user_image = (string) $img;
                $o_data = convert_all_elements_to_string($o_data);
                $status = (string) 1;
                $message = trans('validation.profile_fetched_successfully');
            } else {
                $message = trans('validation.no_data_to_show');
            }
        }
     ...
     public function user_profile(REQUEST $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'user_id.required' => trans('validation.access_token_required'),
        ]);
    ...
    public function add_address(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'latitude.required'       => trans('validation.latitude_required'),
            'longitude.required'      => trans('validation.longitude_required'),
            'location.required'       => trans('validation.location_required'),
            'building_name.required'  => trans('validation.building_name_required'),
            'address.required'        => trans('validation.address_required'),
            'access_token.required'   => trans('validation.access_token_required'),
            'is_default.required'     => trans('validation.is_default_required'),
            'is_default.in'           => trans('validation.is_default_invalid'),
        ]);
        ...
        if ($validator->fails()) {
            ...
            $address->save();
            $status = (string) 1;
            $message = trans('validation.address_added_successfully');
    ...
    public function edit_address(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'address_id.exists'       => trans('validation.address_not_found'),
            'latitude.required'       => trans('validation.latitude_required'),
            'longitude.required'      => trans('validation.longitude_required'),
            'location.required'       => trans('validation.location_required'),
            'building_name.required'  => trans('validation.building_name_required'),
        ...
        if ($validator->fails()) {
            ...
            if (!$address) {
                $status = (string) 0;
                $message = trans('validation.address_not_found');
            } else {
                if ($request->is_default == 1) {
                    $removedefault = UserAdress::where('user_id', $user_id)->update(['is_default' => 0]);
                }
                ...
                $address->save();
                $status = (string) 1;
                $message = trans('validation.address_updated_successfully');
    ...
    public function setdefault(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!$address) {
                ...
                $address->save();
                $status = (string) 1;
                $message = trans('validation.address_set_as_default');
                $o_data = UserAdress::get_address_list($user_id);
                foreach ($o_data as $key => $value) {
                    $o_data[$key]->land_mark = (string)$value->land_mark;
                }
    ...
    public function delete_address(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!$address) {
                ...
                $message = trans('validation.address_deleted_successfully');
                $o_data = UserAdress::get_address_list($user_id);
                foreach ($o_data as $key => $value) {
                    $o_data[$key]->land_mark = (string)$value->land_mark;
                }
    ...
    public function list_address(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user_id = $this->validateAccesToken($request->access_token);
            $status = (string) 1;
            $message = trans('validation.address_fetched_successfully');
            $o_data = UserAdress::get_address_list($user_id);
            foreach ($o_data as $key => $value) {
                $o_data[$key]->land_mark = (string)$value->land_mark;
            }
    ...
    public function update_user_profile(REQUEST $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'user_image.mimes' => trans('validation.invalid_image_format'),
        ]);
        ...
        if ($validator->fails()) {
            ...
            if (!empty($vendordata->logo)) {
                ...
                $userdata->user_image = !$user->user_image ? asset("storage/placeholder.png") : get_uploaded_image_url($user->user_image,'user_image_upload_dir');
            }

            $o_data = $userdata;
            $o_data = convert_all_elements_to_string($o_data);
            $status = (string) 1;
            $message = trans('validation.profile_updated_successfully');
    ...
    public function change_password(REQUEST $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'access_token.required' => trans('validation.access_token_required'),
            'old_password.required' => trans('validation.old_password_required'),
            'new_password.required' => trans('validation.new_password_required'),
            'new_password.min' => trans('validation.password_min_length'),
        ]);
        ...
        if ($validator->fails()) {
            ...
            if (Hash::check($request->old_password, $user->password)) {
                ...
                $user->save();
                $status = (string) 1;
                $message = trans('validation.password_updated_successfully_change');
            } else {
                $message = trans('validation.old_password_incorrect');
            }
        }
        return response()->json(['status' => $status, 'message' => $message, 'errors' => $errors, 'oData' => $o_data]);
    }
    ...
    public function wallet_details(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $wallet_history = \App\Models\WalletHistory::where(['user_id' => $user_id])->orderBy('id', 'desc')->limit($limit)->skip($offset)->get();
    ...
    function favouritesProducts(Request $request)
    {
         App::setLocale($request->header('language') ?? $request->language ?? 'en');
        $status = (string) 1;
        ...
        $check_exist = Likes::where(['user_id' => $user_id])->pluck('product_id')->toArray();
        $where['deleted'] = 0;
        ...
        $favourites_pharmacies = ProductModel::products_list($where, $filter, $limit, $offset,$check_exist)->get();

        $favourites_pharmacies = $this->product_inv($favourites_pharmacies, $user);
         foreach ($favourites_pharmacies as $key => $product) {
            $favourites_pharmacies[$key]['product_name'] = $language == 'ar' ? $product['product_name_arabic'] : $product['product_name'];
        ...
        $favourites_pharmacies =$favourites_pharmacies->count() ? convert_all_elements_to_string($favourites_pharmacies) : [];
    ...
    public function like_dislike(REQUEST $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'id.numeric'            => trans('validation.id_numeric'),
        ...
        if ($validator->fails()) {
            ...
            if ($check_exist->count() > 0) {
                ...
                $status = (string) 1;
                $message = trans('validation.disliked_successfully');;
            } else {
                $like = new Likes();
                ...
                if ($like->id > 0) {
                    $status = (string) 1;
                    $message = trans('validation.liked_successfully');
                } else {
                    $message = trans('validation.failed_to_like');
                }
            }
        }
        return response()->json(['status' => $status, 'error' => $errors, 'message' => $message, 'oData' => $o_data], 200);
    ...
    public function followVendor(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($check_exist->count() > 0) {
                Likes::where([$field => $request->id, 'user_id' => $user_id])->delete();
                $status = (string) 1;
                $message = trans('validation.un_followed');
            } else {
                $like = new Likes();
                $like->vendor_id = $vendor_id ?? 0;
                $like->product_id = $product_id ?? 0;
                ...
                if ($like->id > 0) {
                    ...
                    $message = trans('validation.failed_to_follow');
                }
            }
        }
        return response()->json(['status' => $status, 'error' => $errors, 'message' => $message, 'oData' => $o_data], 200);
    ...
    function followedVendors(Request $request)
    {
         App::setLocale($request->header('language') ?? $request->language ?? 'en');
        $status = (string) 1;
        $message = trans('validation.success');
        ...
        foreach ($favourites_pharmacies as $key => $val) {
            $storeDetails = Stores::where('vendor_id', $val->id)->first();
             if ($storeDetails) {
                $favourites_pharmacies[$key]->logo = $storeDetails->logo;
            } else {
                
                $favourites_pharmacies[$key]->logo = ""; 
            }
            $favourites_pharmacies[$key]->is_liked = 0;
            $favourites_pharmacies[$key]->rating = number_format(0, 1, '.', '');
            if ($user) {
     ...
     public function likeVendor(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user_id = $this->validateAccesToken($request->access_token);
            $check_exist = VendorLikes::where([$field => $request->id, 'user_id' => $user_id])->get();
            if ($check_exist->count() > 0) {
                VendorLikes::where([$field => $request->id, 'user_id' => $user_id])->delete();
                $status = (string) 1;
                $message = trans('validation.un_followed');
            } else {
                $like = new VendorLikes();
                $like->vendor_id = $vendor_id ?? 0;
                $like->user_id = $user_id;
                $like->save();
                if ($like->id > 0) {
    ...
    function likedVendors(Request $request)
    {
         App::setLocale($request->header('language') ?? $request->language ?? 'en');
        $status = (string) 1;
        $message = trans('validation.success');
        ...
        $favourites_pharmacies = VendorDetailsModel::select('vendor_details.user_id as id', 'company_name', 'location', 'logo', 'latitude', 'longitude')
            ->join('vendor_likes', 'vendor_likes.vendor_id', '=', 'vendor_details.user_id')
            ->where('vendor_likes.user_id', $user_id)
            ->get();
        ...
        $o_data['liked']  = $favourites_pharmacies;
        return response()->json(['status' => $status, 'message' => $message, 'errors' => (object) $errors, 'oData' => $o_data], 200);
    }
