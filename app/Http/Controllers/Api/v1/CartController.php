...
class CartController extends Controller
{
    ...
    public function add_to_cart(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // $query = Personalization::where('product_id', $product_id);
            ...
            if ($request->hasFile('customer_file')) {
                $file_name = $this->verifyAndStoreImage(
                    $request,
                    'customer_file',
                    config('global.product_image_upload_dir')
                );
            }
            ...
            if ($product_status && !empty($product_data)) {
                ...
                if ($cart_store_check->count() > 100) {
                    ...
                    $i_data['notes'] = $notes;
                    $i_data['uploaded_file_path'] = $file_name;
    ...
    public function storePersonalization(Request $request)
    {
       ...
       if($request->access_token){
        ...
        $personalization = Personalization::when($user_id, function ($query) use ($user_id) {
            return $query->where('user_id', $user_id);
        })
        ->when(!$user_id, function ($query) use ($cookie_id) {
            return $query->where('cookie_id', $cookie_id);
        })
        ->where('product_id', $validated['product_id'])
        ->first();
    
        $file_name = $personalization->uploaded_file_path ?? null;
        ...
        if ($personalization) {
            ...
            $message = 'Personalization added.';
        }
    
        $data = convert_all_elements_to_string($personalization);
        $customerFileUrl = $data->uploaded_file_path
            ? get_uploaded_image_url($data->uploaded_file_path, 'product_image_upload_dir')
            : '';
        ...
        //     'message' => $message,
        //     'oData' => [
        //         'uploaded_file_path' => $customerFileUrl,
        //         'notes' => $data->notes,
        //         'id' => (string) $data->id,
        ...
        if (!$request->cookie('user_cookie_id')) {
            $response->withCookie(cookie('user_cookie_id', $cookie_id, 60 * 24 * 30)); // 30 days
        }
        return $response;
    }
    ...
    //     $cookie_id = request()->cookie('user_cookie_id');
    
    //     // Build query
    //     $query = Personalization::where('product_id', $validated['product_id']);
    
    //     if ($user_id) {
    //         $query->where('user_id', $user_id);
    //     } elseif ($cookie_id) {
    //         $query->where('cookie_id', $cookie_id);
    //     }
    ...
    //     return response()->json([
    //         'status' => 1,
    //         'message' => 'Personalization data fetched.',
    //         'oData' => [
    //             'uploaded_file_path' => $customerFileUrl,
    //             'notes' => $data->notes,
    //             'id' => (string) $data->id,
    ...
    {
        $validated = $request->validate([
            'access_token' => 'nullable', // access_token is now optional
            'product_id' => 'required|numeric|min:1',
        ]);

        $user_id = 0;
        // If access_token is provided, validate and get user_id, otherwise, leave user_id as 0
        if ($request->access_token) {
            $user_id = $this->validateAccesToken($request->access_token);
        }
        ...
        // Add conditions based on user_id or cookie_id
        if ($user_id) {
            $query->where('user_id', $user_id);
        } elseif ($cookie_id) {
            $query->where('cookie_id', $cookie_id);
    ...
    public function process_cart($user_id,$api='1',$request=null,$is_cart=FALSE)
    {
        ...
        foreach ($cart as $key => $val) {
            ...
            $customerFileUrl = $val->uploaded_file_path
                        ? get_uploaded_image_url($val->uploaded_file_path, 'product_image_upload_dir')
                        : '';
                
            $cart[$key]->product_details = $product;
            $cart[$key]->notes = $val->notes;
            $cart[$key]->customer_file = $val->uploaded_file_path;
            $cart[$key]->uploaded_file_path = $customerFileUrl;
        }
    ...
    public function payment_init(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ((int) $amount_to_pay == 0) {
                ...
                if($vendor->minimum_order_amount && $vendor->minimum_order_amount > $amount_to_pay && 1==2){ // 1==2 just need to remove the condition
                    ...
                    $temp_order->grand_total = ($price_after_discount) + $tax_amount +  $cart_details['service_charge'];
                    if($request->test){
                        // dd($tax_amount,$temp_order);
                    }
                    $admin_commission = $temp_order->service_charge;
                    $vendor_commission = 0;
                    $cart_total = (int)$temp_order->grand_total - (int)$temp_order->vat - $temp_order->service_charge;
                    ...
                    foreach ($cart_details['cart_items'] as $val) {
                        ...
                        $temp_order_prds->customer_notes = $val['notes'] ?? '';
                        $temp_order_prds->customer_file = $val['customer_file'] ?? '';
    ...
    public function place_order(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($res_status === 1) {
                ...
                $o_data['order_no']    = config('global.sale_order_prefix') . date(date('Ymd', strtotime($orderdata->created_at))) . $orderdata->order_id;
                OrderModel::where('order_id', $orderdata->order_id)->update(['order_no' => $o_data['order_no']]);
              //  \Artisan::call("order:send_order_received ".$orderdata->order_id ); 
            } else {
                $status = 0;
        ...
        //try {
            ...
            if ($data) {
                ...
                foreach ($order_prds_data as $val) {
                    ...
                    $existprodct=   ProductModel::find($val->product_id);
                            

                    $store = Stores::where('vendor_id', $existprodct->product_vender_id)->first();

                    // Set fallback values if store not found
                    $adminSharePercent = $store->admin_share ?? 95;
                    $vendorSharePercent = $store->vendor_share ?? 5;
                    $settings = SettingsModel::first();
                    $tax_percentage = isset($settings->tax_percentage) ? $settings->tax_percentage : 5;
                
                    // Apply 5% VAT to product price
                    
                    $priceWithVat = $val->price + ($val->price * ($tax_percentage / 100));
                    $vatPerUnit = $val->price * ($tax_percentage / 100);
                    $totalWithVat = $priceWithVat * $val->quantity;
                    ...
                    $order_prds->customer_notes = $val->customer_notes;  // Save customer notes
                    $order_prds->customer_file = $val->customer_file; 
                    $order_prds->vendor_id = $val->vendor_id;
                    $order_prds->order_status = 0;
                    $order_prds->is_muted = 0;
                    $order_prds->vat_amount = round($vatPerUnit, 2);
                    $order_prds->admin_share = round($adminShareAmount, 2);
                    $order_prds->vendor_share = round($vendorShareAmount, 2);
                     $order_prds->admin_commission = round($adminShareAmount, 2);
                    $order_prds->vendor_commission = round($vendorShareAmount, 2);
                    $order_prds->shipping_charge =0;
                    $order_prds->save();
