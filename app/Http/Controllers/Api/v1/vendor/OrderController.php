...
class OrderController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $orders = OrderModel::select('order_products.*', 'users.name', 'orders.*','users.dial_code','users.phone',
        DB::raw("CONCAT(users.first_name,' ',users.last_name) as customer_name"),'orders.order_id as id',
        'order_products.is_muted as order_product_is_muted',
        'order_products.total as order_product_total'
        ...
        foreach ($orders as $key=>$order){
            ...
            $orders[$key]->status_text = order_status($order->order_status);
            $orders[$key]->status=$order->order_status;
              $vendor_total = OrderProductsModel::where('order_id', $order->order_id)
            ->where('vendor_id', $vendor_id)
            ->sum('total');
            /// Step 1: Calculate percentage of the total
$percentage = $vendor_total / $order->total;

// Step 2: Calculate proportional VAT and grand total
$product_vat = $order->vat * $percentage;
$product_grand_total = $vendor_total + $product_vat;

// Format to 2 decimal places
$product_vat = round($product_vat, 2);
$product_grand_total = round($product_grand_total, 2);
             $orders[$key]->grand_total=$product_grand_total;
             $orders[$key]->is_muted=$order->order_product_is_muted;
            ...
            ->where('order_products.order_id' ,'=',$order->order_id)->where('order_products.vendor_id','=',$vendor_id)->get();
                ...
                if($value['image']) {
                    ...
                    if ($images) {
                        ...
                        foreach ($images as $img) {
                            if ($img) {
                                $prd_img[$i] = get_uploaded_image_url($img,'product_image_upload_dir');;
                                $i++;
                            }
                        }
    ...
    public function order_details(Request $request)
    {
        ...
        $orders = OrderModel::select('order_products.*', 'users.name', 'orders.*','users.dial_code','users.phone', DB::raw("CONCAT(users.first_name,' ',users.last_name) as customer_name"),
        'orders.order_id as id',
        'order_products.total as order_product_total')->join('order_products', 'order_products.order_id', '=', 'orders.order_id')
        ...
        if(!empty($orders))
        {
              ...
              $vendor_total = OrderProductsModel::where('order_id', $orders->order_id)
            ->where('vendor_id', $vendor_id)
            ->sum('total');
             /// Step 1: Calculate percentage of the total
$percentage = $vendor_total / $orders->total;

// Step 2: Calculate proportional VAT and grand total
$product_vat = $orders->vat * $percentage;
$product_grand_total = $vendor_total + $product_vat;

// Format to 2 decimal places
$product_vat = round($product_vat, 2);
$product_grand_total = round($product_grand_total, 2);
        $orders->status_text = order_status($orders->order_status);
        $orders->grand_total = $product_grand_total;
         $orders->vat = $product_vat;
         $orders->total = $vendor_total ;
        //$orders[$key]->status_text = order_status($order->order_status);
        ...
        $orders->booking_date = get_date_in_timezone($orders->booking_date, 'Y-m-d H:i:s');
            ...
            $orderProductDetail  = OrderProductsModel::leftjoin('product as p','p.id','=','order_products.product_id')
                ...
                'pas.image')
            ->leftjoin('product_selected_attribute_list as pas','pas.product_attribute_id','=','order_products.product_attribute_id')
            ->where('order_products.order_id' ,'=',$orders->order_id)->where('order_products.vendor_id','=',$vendor_id)->get()->toArray();
            $orderProductDetail = OrderProductsModel::where('order_products.order_id', $orders->order_id)
    ->where('order_products.vendor_id', $vendor_id)
    ->get();

$finalProductDetail = [];

foreach ($orderProductDetail as $val) {
    ...
    if ($status && $product) {
        ...
        // Add custom order details to product
        ...
        $product['customer_file_name'] = $val->customer_file?? "";
        $product['customer_file_url'] = $customerFileUrl?? "";

        $finalProductDetail[] = $product;
    }
}

// Merge product data into order
$o_data = $orders;
$o_data['productDetail'] = $finalProductDetail;
        ...
        return response()->json(['status' => (string)$status, 'error' => $errors, 'message' => $message, 'oData' => $o_data], 200);
    ...
    public function change_status(OrderChangeStatusRequest $request)
    {
        ...
        OrderProductsModel::where('order_id',$request->order_id)->where('vendor_id',$userid)
        ->update(
            [
    ...
    $status   = "0";
     ...
     if ($validator->fails()) {
         ...
         if($type == 'all'){
             ...
             ->update(['is_muted'=>1]);
             $list = OrderProductsModel::where('vendor_id',$userid)->update(['is_muted'=>1]);
             $status = "1";
             ...
             ->update(['is_muted'=>1]);
             $list = OrderProductsModel::where('order_id',$request->order_id)->where('vendor_id',$userid)->update(['is_muted'=>1]);
             $status = "1";
             $message = "order muted";
         }
     }
    ...
    private function validateAccesToken($access_token)
    {
        ...
        if ($user->count() == 0) {
            ...
            $user = $user->first();
            if ($user->verified == 1 || $user->role!=2) {
                return $user->id;
            } else {
                http_response_code(401);
                echo json_encode([
