        ...
        if ($validator->fails()) {
            ...
            if ($data->count() > 0) {
                ...
                $img = asset("storage/placeholder.png");
                 ...
                 $store = Stores::where('vendor_id', $user_id)->first(); 
                   
                    
                
                $o_data->user_image =  $store->logo;
               $o_data->dob=$store->dob;
               $o_data->description_ar=$store->description_ar;
           $o_data->description=$store->description;
                $o_data = convert_all_elements_to_string($o_data);
                $status = "1";
                $message = "data fetched Successfully";
            } else {
    ...
    public function update_user_profile(REQUEST $request){
      ...
      if ($validator->fails()) {
        ...
        if($file = $request->file("user_image")){
                    ...
                    $store = Stores::where('vendor_id', $user->id)->first(); 
                    $store->logo=$response['link'];
                    $store->save();
     ...
     if (!empty($request->dob)) {
           ...
           $store->dob_day = $dob ? $dob->format('d') : null;
            $store->dob_month = $dob ? $dob->format('m') : null;
           $store->dob_year = $dob ? $dob->format('Y') : null;
