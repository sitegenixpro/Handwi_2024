...
class BlogsController extends Controller
{
     ...
     */
     public function index()
     {
        ...
        $page_heading = "Blogs";
        $testimonial = Blog::where(['deleted' => 0])->orderby('id','desc')->get();

        return view('admin.blogs.list', compact('page_heading', 'testimonial'));
     }
     public function create($id='')
    {
        ...
        if($id > 0)
        {
          $testimonial = Blog::where(['id'=>$id])->first();
          if ($testimonial) {
              $page_heading = "Blogs ";
              $mode = "edit";
        ...
        return view("admin.blogs.create", compact('page_heading','testimonial', 'id'));
    }
     public function store(Request $request)
     {
         ...
         if ($validator->fails()) {
             ...
             $input = $request->all();
             $check_exist = Blog::where(['deleted' => 0, 'name' => $request->name])->where('id', '!=', $request->id)->get()->toArray();
             if (empty($check_exist)) {
                ...
                //     $ins['blog_image'] = $file_name;
                // }
                //  if ($file = $request->file("image")) {
                //     $response = image_upload($request,config('global.banner_image_upload_dir'),'image');
                ...
                if ($file = $request->file("image")) {
                 ...
                 if ($request->id != "") {
                    ...
                    Blog::where('id',$request->id)->update($ins);
                     $news_id = $request->id;
                     $status = "1";
