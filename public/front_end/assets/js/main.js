...
// Hide filterbar on document click, except when clicking inside .filterbar or on .btn-filter or on inputs inside .filterbar
$("body").on('click', function (event) {
    var $target = $(event.target);
    
    if (
        !$target.closest('.filterbar').length &&  // click is outside filterbar
        !$target.is('.btn-filter') &&             // not the filter toggle button
        !$target.closest('.select2-search__field').length && // not an input inside filterbar
        !$target.closest('.select2').length // not an input inside filterbar
    ) {
        $(".filterbar").removeClass("active");
    }
});
